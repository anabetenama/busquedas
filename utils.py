...
import math #Añadir un import para que haga la operación de math
infinity = 1.0e400
...
class Heuristica(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda x: x.path_cost+self.problem.h(x)) #Criterio por el que se ordena (x.path_cost)
